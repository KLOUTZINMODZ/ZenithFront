...
import { ShoppingBag, ShoppingCart, Search, Filter, ArrowUpDown } from 'lucide-react';
...
import { OrderCard, PurchaseOrder } from '../components/ui/OrderCard';
...
const StatsSummary: React.FC<{ stats: PurchaseStats }> = ({ stats }) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      >
        <h3 className="text-gray-400 text-sm mb-1">Compras concluídas</h3>
      ...
      >
        <h3 className="text-gray-400 text-sm mb-1">Compras em andamento</h3>
      ...
      >
        ...
        <p className="text-white text-2xl font-bold">{formatCurrency(stats.totalSpent, true)}</p>
      </motion.div>
    </div>
  );
};
...
const PurchasesPage: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const orders = response.data.orders as PurchaseOrder[];
        setOrders(orders);
        ...
        if (allOrdersResponse.success && allOrdersResponse.data) {
          const allOrders = allOrdersResponse.data.orders as PurchaseOrder[];
      ...
      setError('Erro de conexão ao carregar compras');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    const onMpStatus = (_data: any) => { try { setRefreshTrigger(v => v + 1); } catch {} };
    const onNotif = (payload: any) => {
      try {
        const t = payload?.notification?.type || payload?.type;
        if (t && ['purchase:initiated', 'purchase:completed', 'purchase:cancelled', 'purchase:shipped'].includes(String(t))) {
  ...
  // Função para avaliar uma compra concluída
  const handleRateOrder = (orderId: string, type?: string) => {
    // Para boosting, precisamos buscar o agreementId correto
    if (type === 'boosting') {
      // Encontrar a ordem para obter o agreementId
      const order = orders.find(o => o._id === orderId);
      const boostingAgreementId = (order as any)?.agreementId || order?.item?._id;
      const fallbackId = order?.orderNumber || orderId;
      navigate(`/rate/boosting/${boostingAgreementId || fallbackId}`);
    } else {
      navigate(`/rate/${type || 'marketplace'}/${orderId}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <PageHeader 
        ...
        description="Acompanhe seus pedidos e status das compras"
        icon={ShoppingCart}
      ...
      {loading && (
        <div className="bg-gray-800/90 rounded-xl border border-gray-700/40 p-6">
          <LoadingState text="Carregando seus pedidos..." />
        </div>
      )}
      ...
      {!loading && !error && orders.length === 0 && (
        <EmptyState
          ...
          description="Você ainda não realizou nenhuma compra no marketplace."
